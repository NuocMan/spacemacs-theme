...
(defun create-spacemacs-theme (variant theme-name)
  (let* (;; Helper function to get custom color or default value
         ...
         (pick-color (lambda (dark-gui light-gui)
                       (if (eq variant 'dark) dark-gui light-gui)))
         (class '((class color) (min-colors 89)))
         ;;                                                                   - Dark -- - Light -
         (act1          (funcall get-color 'act1 (funcall pick-color          "#222226" "#e7e5eb")))
         (act2          (funcall get-color 'act2 (funcall pick-color          "#5d4d7a" "#d3d3e7")))
         (base          (funcall get-color 'base (funcall pick-color          "#b2b2b2" "#655370")))
         (base-dim      (funcall get-color 'base-dim (funcall pick-color      "#686868" "#a094a2")))
         (bg1           (funcall get-color 'bg1 (funcall pick-color           "#292b2e" "#fbf8ef")))
         (bg2           (funcall get-color 'bg2 (funcall pick-color           "#212026" "#efeae9")))
         (bg3           (funcall get-color 'bg3 (funcall pick-color           "#100a14" "#e3dedd")))
         (bg4           (funcall get-color 'bg4 (funcall pick-color           "#0a0814" "#d2ceda")))
         (bg-alt        (funcall get-color 'bg-alt (funcall pick-color        "#42444a" "#efeae9")))
         (border        (funcall get-color 'border (funcall pick-color        "#5d4d7a" "#b3b9be")))
         (cblk          (funcall get-color 'cblk (funcall pick-color          "#cbc1d5" "#655370")))
         (cblk-bg       (funcall get-color 'cblk-bg (funcall pick-color       "#2f2b33" "#e8e3f0")))
         (cblk-ln       (funcall get-color 'cblk-ln (funcall pick-color       "#827591" "#9380b2")))
         (cblk-ln-bg    (funcall get-color 'cblk-ln-bg (funcall pick-color    "#373040" "#ddd8eb")))
         (cursor        (funcall get-color 'cursor (funcall pick-color        "#e3dedd" "#100a14")))
         (const         (funcall get-color 'const (funcall pick-color         "#a45bad" "#4e3163")))
         (comment       (funcall get-color 'comment (funcall pick-color       "#2aa1ae" "#2aa1ae")))
         (comment-light (funcall get-color 'comment-light (funcall pick-color "#2aa1ae" "#a49da5")))
         (comment-bg    (funcall get-color 'comment-bg (funcall pick-color    "#292e34" "#ecf3ec")))
         (comp          (funcall get-color 'comp (funcall pick-color          "#c56ec3" "#6c4173")))
         (err           (funcall get-color 'err (funcall pick-color           "#e0211d" "#e0211d")))
         (func          (funcall get-color 'func (funcall pick-color          "#bc6ec5" "#6c3163")))
         (head1         (funcall get-color 'head1 (funcall pick-color         "#4f97d7" "#3a81c3")))
         (head1-bg      (funcall get-color 'head1-bg (funcall pick-color      "#293239" "#edf1ed")))
         (head2         (funcall get-color 'head2 (funcall pick-color         "#2d9574" "#2d9574")))
         (head2-bg      (funcall get-color 'head2-bg (funcall pick-color      "#293235" "#edf2e9")))
         (head3         (funcall get-color 'head3 (funcall pick-color         "#67b11d" "#67b11d")))
         (head3-bg      (funcall get-color 'head3-bg (funcall pick-color      "#293235" "#edf2e9")))
         (head4         (funcall get-color 'head4 (funcall pick-color         "#b1951d" "#b1951d")))
         (head4-bg      (funcall get-color 'head4-bg (funcall pick-color      "#32322c" "#f6f1e1")))
         (highlight     (funcall get-color 'highlight (funcall pick-color     "#444155" "#d3d3e7")))
         (highlight-dim (funcall get-color 'highlight-dim (funcall pick-color "#3b314d" "#e7e7fc")))
         (keyword       (funcall get-color 'keyword (funcall pick-color       "#4f97d7" "#3a81c3")))
         (lnum          (funcall get-color 'lnum (funcall pick-color          "#44505c" "#a8a8bf")))
         (mat           (funcall get-color 'mat (funcall pick-color           "#86dc2f" "#ba2f59")))
         (meta          (funcall get-color 'meta (funcall pick-color          "#9f8766" "#da8b55")))
         (str           (funcall get-color 'str (funcall pick-color           "#2d9574" "#2d9574")))
         (suc           (funcall get-color 'suc (funcall pick-color           "#86dc2f" "#42ae2c")))
         (ttip          (funcall get-color 'ttip (funcall pick-color          "#9a9aba" "#8c799f")))
         (ttip-sl       (funcall get-color 'ttip-sl (funcall pick-color       "#5e5079" "#c8c6dd")))
         (ttip-bg       (funcall get-color 'ttip-bg (funcall pick-color       "#34323e" "#e2e0ea")))
         (type          (funcall get-color 'type (funcall pick-color          "#ce537a" "#ba2f59")))
         (var           (funcall get-color 'var (funcall pick-color           "#7590db" "#715ab1")))
         (war           (funcall get-color 'war (funcall pick-color           "#dc752f" "#dc752f")))
         (aqua          (funcall get-color 'aqua (funcall pick-color          "#2d9574" "#2d9574")))
         (aqua-bg       (funcall get-color 'aqua-bg (funcall pick-color       "#293235" "#edf2e9")))
         (green         (funcall get-color 'green (funcall pick-color         "#67b11d" "#67b11d")))
         (green-bg      (funcall get-color 'green-bg (funcall pick-color      "#293235" "#edf2e9")))
         (green-bg-s    (funcall get-color 'green-bg-s (funcall pick-color    "#29422d" "#dae6d0")))
         (cyan          (funcall get-color 'cyan (funcall pick-color          "#28def0" "#21b8c7")))
         (red           (funcall get-color 'red (funcall pick-color           "#f2241f" "#f2241f")))
         (red-bg        (funcall get-color 'red-bg (funcall pick-color        "#3c2a2c" "#faede4")))
         (red-bg-s      (funcall get-color 'red-bg-s (funcall pick-color      "#512e31" "#eed9d2")))
         (blue          (funcall get-color 'blue (funcall pick-color          "#4f97d7" "#3a81c3")))
         (blue-bg       (funcall get-color 'blue-bg (funcall pick-color       "#293239" "#edf1ed")))
         (blue-bg-s     (funcall get-color 'blue-bg-s (funcall pick-color     "#2d4252" "#d1dcdf")))
         (magenta       (funcall get-color 'magenta (funcall pick-color       "#a31db1" "#a31db1")))
         (yellow        (funcall get-color 'yellow (funcall pick-color        "#b1951d" "#b1951d")))
         (yellow-bg     (funcall get-color 'yellow-bg (funcall pick-color     "#32322c" "#f6f1e1"))))
